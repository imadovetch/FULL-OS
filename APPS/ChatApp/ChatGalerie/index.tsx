...
export default function ChatGalery({data,status , getphoto}){
  ...
  const handlePhotoClick = (photoId) => {
    ...
    return(
     <div className={` pb-4 grid ${(data.width > 800 && data.width<1000) ? ' grid-cols-1' : 'grid-cols-2'}  custom-scrollbar  gap-2 absolute bottom-5 p-1  right-0 ${(data.fullscreen || (data.width > 1000)) ? 'w-2/5' : ((data.width > 800 ) ? 'w-1/5' : 'w-3/5')}  h-4/5 Chatphotoholder z-50 ${status ? '' : 'hidden'}`}>
           {photos.map((photo) => (
